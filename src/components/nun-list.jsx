...
class NunsList extends Component {
  ...
  componentDidMount() {
    getAllNuns().then((nuns) => {
      console.log("componentDidMount");
      this.setState({ nuns: nuns });
    });
  }
    ...
    return (
      <React.Fragment>
        ...
        {!this.state.showEditForm ? (
          <React.Fragment>
            {" "}
            <div className="nun-list_header_buttton">
              {!this.state.tableView ? (
                <IconButton onAction={this.handleViewInfo} icon={"BsTable"} />
              ) : (
                <IconButton
                  onAction={this.handleViewInfo}
                  icon={"BsGrid3X3Gap"}
                />
              )}
            </div>
            <React.Fragment>
              ...
              {this.state.tableView ? (
                ...
                <div className="nun-card_content">
                  {this.state.nuns &&
                    this.state.nuns.map((currentNun) => (
                      <NunCard
                        nun={currentNun}
                        onAction={this.handleChangeEdit}
                      />
                    ))}
                </div>
              )}
